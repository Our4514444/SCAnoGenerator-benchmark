...
COMMAND = "slither "
LOG_TXT = "log.txt"
BUG_TYPE1 = "Reentrancy in"
BUG_TYPE2 = ""
BUG_PREFIX = "ruleId: "
HUANGGAI_INFOTXT_SUFFIX = "Info.txt"
SOLIDIFI_INFOTXT_SUFFIX = ".csv"
STOP_FLAG1 = "Reference: https://github.com/"
STOP_FLAG2 = "State variables written after the"

def getBugLocation(_resultList, _bugList):
	#这一部分根据smartcheck的输出定制
	#对于每一个要捕获的bug
	locationList = list()
	lineNumPattern = re.compile(r"(\.)sol(#)(.)+(\))")
	for bug in _bugList:
		#遍历每一行
		for (index, line) in enumerate(_resultList):
			if bug in line:
				...
				while bug not in infoLine and STOP_FLAG1 not in infoLine and STOP_FLAG2 not in infoLine:
					#print(infoLine)
					#此行是否包含行号
					if lineNumPattern.search(infoLine):
						#包含行号, 提取行号
						lineStr = lineNumPattern.search(infoLine).group()
						lineNum = lineStr[lineStr.find("#") + 1:lineStr.find("-")]
						#try:
	...
	if os.path.exists(filename + SOLIDIFI_INFOTXT_SUFFIX):
		...
		with open(infoFilename, "r") as f:
			...
			for row in list(reader)[1:]:
				aBugLocation = list(range(int(float(row[0])), int(float(row[0])) + int(float(row[1]))))
...
# 	for contract in track(solList, description="Slither is working..."):
# 		#切换编译器版本
# 		print(contract)
...
# 		#实际上就是求交集
# 		print("---------------------------")
# 		print(set(bugLocationList))
# 		print(set(injectBugLocationList))
...
# 	_contractName = './buggy_0xeaf28dc83537cd526f703ee7157a55beac9e073f_reentrancy.sol'
# 	for filename in os.listdir():
# 		if filename.endswith('.sol'):
# 			fi = getInjectLocationForSolidiFI(_contractName)
# 			print(fi)
