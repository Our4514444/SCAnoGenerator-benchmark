...
manticore要设置超时时间
manticore新挑战, 要指定主类
manticore半天产生不了一个结果, 这样, 先跑，记录每个合约与其的分析结果
然后等全部跑完了, 再看是否有有用的结果
'''
...
import time
from inherGraph import inherGraph	#用于返回主类名称
import json

#使用的常量
COMMAND = "manticore "
COMMAND_SUFFIX = " --contract "
JSON_AST_NAME = "."
LOG_TXT = "log.txt"
BUG_TYPE1 = "107"
BUG_TYPE2 = "120"
...
STOP_FLAG2 = "SWC ID: "
STOP_FLAG3 = "Caller:"
RESULT_STORE_LOCATION = "Results in "
RESULT_STORE_FILE = "contractAndItsResult.json"
...
def changeSolcVersion(_sourceCode):
	...
	#否则使用默认声明
	try:
		compileResult = subprocess.run("solc use " + defaultSolc, check = True, shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE)	#切换版本			
	except Exception as e:
		#切换编译器失败，则终止运行
		print(e)
		raise Exception("Failed to switch the solc version.")

def getJsonAstName(_contractName):
	return _contractName + "_json.ast"

def getResultLocation(_resultList):
	for line in _resultList:
		if RESULT_STORE_LOCATION in line:
			#找到说明结果存放位置的那一行
			#分离出那一行
			print()
			resultLocation = line.split()[-1]
...
def main(_usedTool):
	...
	resultStoreDict = dict()	#分析结果存放对应关系
	#2. 然后，调用smartcheck检测每个合约
	for contract in track(solList, description="manticore is working..."):
		try:
			#切换编译器版本
			sourceCode = open(contract, "r").read()
			changeSolcVersion(sourceCode)
			#要先编译出ast
			subprocess.run("solc --ast-json --overwrite " + contract + " -o " + JSON_AST_NAME, check = True, shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
			#然后读取json
			compileJson = str()
			with open(getJsonAstName(contract), "r", encoding = "utf-8") as f:
				compileJson = json.loads(f.read())
			#然后声明需要使用的类
			IG = inherGraph(compileJson)
			#print(IG.getMainContractName())
			#3. 记录检测的输出, 要重定向终端输出
			...
			#重组命令
			#print(COMMAND + contract + COMMAND_SUFFIX + IG.getMainContractName())
			process = subprocess.Popen(COMMAND + contract + COMMAND_SUFFIX + IG.getMainContractName(), shell = True, preexec_fn = os.setpgrp, stdout = detectResultLog, stderr = detectResultLog)
			...
			process.communicate(timeout = 300)	#设置超时时间
			detectResultLog.close()
			#4. 然后读取检测结果, 解析出检测到的bug类型
			#首先是 合约功能受矿工影响　bug
			#传入检测结果和所需的bug类型列表, 返回位置列表
			#只会记录分析不超时的
			resultStoreDict[contract] = getResultLocation(open(LOG_TXT, "r").readlines())	#根据终端输出, 获知结果存在哪里了
			bugLocationList = list()	#先空置
			'''
			...
			if _usedTool == "HG":
				...
				injectedBugSet = set()
				#print(bugLocationList)
				#print(injectBugLocationList)
				#对于每一个发现的bug而言
				for bugLine in bugLocationList:
					#查看它是否属于那一个代码段
					#如果在, 将bugLine加入捕获集合
					for snippetLine in injectBugLocationList:
						if bugLine in snippetLine:
							injectedBugSet.add(bugLine)
							break
						else:
							continue
			...
			injectBugNum += len(injectBugLocationList)
		#print(contract)
	#print(resultStoreDict)
	#保存结果
	with open(RESULT_STORE_FILE, "w") as f:
		f.write(json.dumps(resultStoreDict, indent = 1))
	print("分析结果已保存.")
