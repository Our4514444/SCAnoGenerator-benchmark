...
class canItBeCompiled:
	...
	def storeResult(self, _filename):
		try:
			#使用cp命令拷贝两份结果
			desCode = os.path.join(SUCCESS_PATH, _filename)
			#若文件存在，则不覆盖　
			if os.path.exists(os.path.join(desCode)):
				raise Exception("The result already exists.")
			cutPrefix = _filename.split("_", 1)[1]
			#分离文件名和后缀
			filename = os.path.splitext(cutPrefix)[0]
			#然后拼接头尾
			filename = "BugLog_" + filename + ".csv"
			#执行拷贝，显示详细信息
			codeExecuteResult = subprocess.run("cp -v " + _filename + " " + desCode, check = True, shell = True)
			astExecuteResult = subprocess.run("cp -v " + filename + " " + os.path.join(SUCCESS_PATH, filename), check = True, shell = True)
			return
		except Exception as e:
			print(e)
