...
class canItBeCompiled:
	...
	#掐头去尾，头以_为分界线，尾巴不要后缀
	def getLogFileName(self, _solFileName):
		cutPrefix = _solFileName.split("_", 1)[1]
		#分离文件名和后缀
		filename = os.path.splitext(cutPrefix)[0]
		#然后拼接头尾
		return "BugLog_" + filename + ".csv"
	...
	def extractContracts(self):
		...
		totalBugNum = 0	#注入bug的总数
		failBugNum = 0	#注入失败的bug总数
		successContractList = list()	#被成功编译的合约列表
		while self.nowNum < self.needs and self.index < self.maxIndex:
			...
			try:
				...
				#print(prevFileName)
				#然后找到对应的bug注入文件
				logFileName = self.getLogFileName(prevFileName)
				#读取对应的csv文件，除了第一行标题，其余都是注入的bug位置
				with open(logFileName, "r") as f:
					csvReader = csv.reader(f)
					#读取bug数量
					for row in csvReader:
						thisFileBugNum += 1
				thisFileBugNum -= 1	#减去标题行数
				totalBugNum += thisFileBugNum
				#将当前合约暂存
				...
				successNum += 1
				successContractList.append(prevFileName)
				#清空缓存数据
				rmtree(CACHE_PATH)
				#重新建立文件夹
				os.mkdir(CACHE_PATH)
		...
		print("Injecting rate: ", (totalBugNum - failBugNum) / totalBugNum, failBugNum, totalBugNum)
		return successContractList
	...
	def changeSolcVersion(self, _sourceCode):
		...
		try:
			if self.inStandardVersion(self.defaultSolc):
				#在本机支持的solc版本范围内
				compileResult = subprocess.run("solc-select use " + self.defaultSolc, check = True, shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE)	#切换版本				
	...
	def storeResult(self, _filename):
		try:
			...
			#执行拷贝，显示详细信息
			codeExecuteResult = subprocess.run("cp -v " + self.cacheContractPath + " " + desCode, check = True, shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
			astExecuteResult = subprocess.run("cp -v " + self.cacheJsonAstPath + self.cacheJsonAstName + " " + desJsonAst, check = True, shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
			return
		except:
...
if __name__ == "__main__":
	userNeed = sys.argv[1]
	cibc = canItBeCompiled(int(userNeed))
