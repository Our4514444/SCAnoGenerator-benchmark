...
COMMAND = "slither "
LOG_TXT = "log.txt"
BUG_TYPE1 = "uses tx.origin for authorization"
BUG_TYPE2 = ""
BUG_PREFIX = "ruleId: "
...
def getBugLocation(_resultList, _bugList):
	...
	#对于每一个要捕获的bug
	locationList = list()
	lineNumPattern = re.compile(r"(\.)sol(#)((\d)|(-))+(\))$")
	for bug in _bugList:
		#遍历每一行
		for (index, line) in enumerate(_resultList):
			if bug in line:
				...
				while STOP_FLAG1 not in infoLine: #bug not in infoLine and STOP_FLAG1 not in infoLine and STOP_FLAG2 not in infoLine:
					#print(infoLine)
					#此行是否包含行号
					if lineNumPattern.search(infoLine):
						#包含行号, 提取行号
						lineStr = lineNumPattern.search(infoLine).group()
						lineNum = lineStr[lineStr.find("#") + 1:lineStr.find("-")]	#如何没有-分隔号, 则默认返回-1
						#try:
