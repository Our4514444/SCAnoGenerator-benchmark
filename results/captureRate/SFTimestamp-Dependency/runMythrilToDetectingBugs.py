	...
	for bug in _bugList:
		...
		for (index, line) in enumerate(_resultList):
			if (BUG_PREFIX + bug) in line:
				...
				while STOP_FLAG1 not in infoLine and STOP_FLAG2 not in infoLine: 
					#print(infoLine)
					#此行是否包含行号
					if lineNumPattern.search(infoLine):
						#包含行号, 提取行号
						lineStr = lineNumPattern.search(infoLine).group()
						lineNum = lineStr[lineStr.find(".sol:") + 5:lineStr.find("-")]	#如何没有-分隔号, 则默认返回-1
						#try:
...
def main(_usedTool):
	...
	for contract in track(solList, description="Mythril is working..."):
		...
		try:
			...
			process.communicate(timeout = 120)	#设置超时时间
